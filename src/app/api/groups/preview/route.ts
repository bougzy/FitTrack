...
export async function GET(req: NextRequest) {
  try {
    ...
    // Find group by invite code — no auth required to preview
    const group = await Group.findOne({
      inviteCode: code.toUpperCase().trim(),
    })
      .select('name description privacy members totalWorkouts inviteCode _id')
    ...
    if (!group) {
      return NextResponse.json(
        { success: false, error: 'Invalid invite code. This group does not exist.' },
        { status: 404 }
      );
    }

    // Check if user is already a member (only if token provided)
    let alreadyMember = false;
    const token = extractTokenFromHeader(req.headers.get('authorization'));
    if (token) {
      try {
        const payload = verifyToken(token);
        alreadyMember = group.members.some((m: any) => {
          return m.userId?.toString() === payload.userId;
        });
      } catch {
        // Token invalid or expired — that's fine, just not a member
        alreadyMember = false;
      }
    }
