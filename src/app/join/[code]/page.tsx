...
export default function JoinGroupPage() {
  ...
  useEffect(() => {
    ...
    if (!isAuthenticated) {
      // Save code so it survives login/register redirect
  ...
  const fetchGroupPreview = async () => {
    ...
    try {
      // Call preview directly with fetch so we control headers precisely
      const storedToken = useAuthStore.getState().token;
      const res = await fetch(
        `/api/groups/preview?code=${encodeURIComponent(code.toUpperCase().trim())}`,
        {
          headers: {
      ...
      if (data.success) {
        ...
        setAlreadyMember(data.data.alreadyMember);
        setStatus('found');
      } else {
      ...
      setErrorMsg('Could not load group info. Check your connection.');
      setStatus('error');
    }
  };
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setErrorMsg(data.error || 'Could not join. Please try again.');
        setStatus('error');
      }
    } catch (err) {
    ...
    return (
      <div className="min-h-screen bg-dark-950 flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-dark-400 text-sm">Loading group invite...</p>
        </div>
      </div>
    );
  }
        ...
        >
          ...
          {alreadyMember ? (
            <div className="space-y-3">
              ...
              <button
                onClick={() => router.replace(`/groups/${group?._id}`)}
                className="w-full py-4 bg-gradient-to-r from-brand-500 to-brand-600 text-white font-display font-bold rounded-2xl active:scale-95 transition-all flex items-center justify-center gap-2"
              >
                Open Group →
              </button>
            </div>
